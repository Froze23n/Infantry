...
PID_T chasZ = {6.0f, '\0' ,40.114514f}; //底盘旋转

PID_T yawA = {30.114514f , '\0' , 10}; //yaw轴角度
PID_T yawV = {781.14514f , 10 , '\0'}; //yaw轴速度

PID_T pitchA = {150.0f , 6 , 10.0f}; //pitch电机
PID_T pitchV = {900.0f , 10.0f , '\0'}; //pitch电机
...
float Chas_Calc_Z(float relative_angle)
{
    ...
    if(outLevel<-3){outLevel=-3;}
    //返回结果
    //if(-0.01<outLevel && outLevel<0.01){outLevel=0;}
    return outLevel;
}
...
#define PITCH_V2V_I_LIMIT (10.0f)
...
int16_t Loader_M2006_PID(float pError)
{
    ...
    if(output <-10000){output =-10000;} /* -10000mV  电流  +10000mV*/
    return (int16_t)output;
}
